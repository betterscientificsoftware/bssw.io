...
While there are a wide variety of "best practices" that can be adopted when developing a software project, an important subset include *coding conventions* which impacts the internal source code in ways that generally does **not** influence the behavior of the software.
...
* **Paradigm- and language-specific guidelines**: Other guidelines that more specific to a given programming paradigm or programming language
...
After formatting and naming guidelines, there are many *other general guidelines* that are fairly universal to all programming paradigms and languages such as for how to write good code comments, elimination of duplication (i.e. don't repeat yourself), and KISS (keep it simple, stupid).
...
* **foster shared ownership of the codebase** (by using the same style one can't tell who wrote the code initially and therefore does not imply ownership),
...
* **provide targets for refactorings** (by helping to highlight code smells that violate the accepted conventions).
...
(In fact, as major part of peer code review is to use expert human judgment is to evaluate code against naming and other general coding guidelines that would be very hard to write a robust automated tool to check.)
...
* However, **a software development project should still adopt an a set coding conventions even in these areas** (and what is important is that a set of conventions are agreed upon and adopted, not so much the details or the choices that are made).
* **Everyone should adhere the currently accepted coding conventions** for the project.
...
As a postscript, note that the Wikipedia page *Coding conventions* does not even mention the Fortran programming language in the section "Coding conventions for languages" (which only covers about 17 languages at the time of this writing) but a simple web search for "fortran coding conventions" or "fortran coding standards" will yield a number of hits for different groups and organizations proving their opinions.
