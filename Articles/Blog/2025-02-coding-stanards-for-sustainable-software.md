...
In addition, many software developers aren’t taught practices that help create more sustainable software.
Managers of software development projects aren’t sure how to measure their success.
...
We hope to demonstrate various ideas by a fictional story of your onboarding experience at Fake Science and Engineering Company (FSEC), where you have been hired to work on a scientific software project.


### Fake Science and Engineering Company (FSEC)

Welcome to FSEC.
You have been hired as a software engineer to work on our latest scientific software project.
...
The next point is very important, and we’ll start with a quote from Martin Fowler: “Any fool can write code that a computer can understand.
Good programmers write code that humans can understand.”

As you work on the project, it’s more important to make the software readable (which leads to sustainability) than it is to get the software merely working.
...
Next, we’ll delve into more details on each one.
...
You’ll learn about the Red, Green, Refactor cycle and refactoring practices to apply.
...
Additionally, you’ll learn about the “Four Rules of Simple Design” from Kent Beck, which will help you execute your refactoring cycle.
...
You’ll also be trained on the S.O.L.I.D.
...
Following Scrum itself is not the sole goal or indicator of developing sustainable software; it’s an integral part of delivering valued software to our customers.
...
This refactoring can occur within the TDD cycle or when we touch legacy code, i.e., code that doesn’t have automated verification like unit tests.
As one measure of success, we’ll measure our technical debt.
...
Teams that don’t use TDD think it helps minimize T1.
They don’t spend time refactoring (T2), hoping it gives them more new development time (T1).
However, experience shows this is a false hope.
Early at FSEC, we realized that many teams didn’t know how efficient their software development processes were.
Hence, managers and team members did not even know this was a problem.
Customers often did not receive the capabilities they asked for (in a timely manner) or found the development expenses unrealistic or didn’t establish trust with the software due to usability issues.

You will be on ONE Scrum team, and you will be 100% dedicated to that team.
We don’t encourage multitasking at FSEC since research and experience show that it immediately reduces an individual’s capacity by 40%.
...
On many teams, T3/T is 50% or more, and as the code base grows the technical debt increases with each new feature, and the time to develop new features (T1) grows ever smaller.
...
We have experienced this firsthand, and hence, our practice is: “if it needs to be maintained, it shall be pair-programmed.” No, it’s not paying two people to do the work of one.

To facilitate this practice, at FSEC we all use the same software development IDE, configured uniformly for everyone.
...
In the Navigator/Driver pair-programming pattern, the individual who knows what code needs to be written doesn’t touch the keyboard (or mouse) and acts as the Navigator.
...
Exceptions exist, mostly in cases where a large conditional or switch statement is necessary, and six lines can’t be accommodated.
However, for most of our code, you’ll be expected to keep the line count to a maximum of six.
Note that curly braces don’t count.
...
This is also explained well in "Clean Code." At FSEC, we require readable code without the need for comments.
Comments become part of the code maintenance, and it’s hard enough to maintain the code itself.
...
Using the refactoring training you’ve received, you will be expected to add unit tests to cover the legacy code before you start modifying the code.
...
One of your main responsibilities as a developer is to NOT create a system test that will significantly affect our continuous integration (CI) times.
...
T2 can be an indicator of how much legacy code we have, and it hopefully isn’t increasing over time.
...
We don’t avoid this area because it’s hard; we do it to ensure we are not flying blind and have a clear sense of how efficiently we are executing our software development projects.
...
We don’t expect everyone to be fungible in domain expertise, but we do expect fungibility in implementing code.
...
At this point in the article, we are concluding our fictional FSEC onboarding story.
How can we move forward in improving our abilities to write sustainable software? Here are some considerations:
...
What if we used the Sustainable Scientific Software Conference (S3C) as a collaborative platform to discuss and develop standards that most organizations could use as a starting point for their own standards and practices? If so, please consider attending and supporting this initiative.
...
This measure isn’t meant to be perfect, but it will give you an idea of where you stand.
Don’t be surprised if it’s around 50%.
...
Why do so many organizations avoid addressing this? If your organization has multiple software development projects, shouldn’t we strive for consistency in the quality of code being developed and delivered? This requires courageous leadership.
After all, who wants to be the one to tell their software development teams that this list is the minimum quality criteria? Consider making a team event to create the initial list.
...
While there isn’t excellent scientific research on this topic, we shouldn’t be flying blind.
